...
namespace Turbina.Editors.Ropes
{
    internal class RopeSimulation
    {
            ...
            )
        {
            ...
            {
                Masses.Add(new Mass(m) {Pos = position });
            }
...
//            var lengthSquared = Math.Pow(Math.Min(Math.Abs(Mass1Position.X-Mass2Position.X)/Math.Abs(Mass1Position.Y - Mass2Position.Y), 100), 2);
            var lengthSquared = Math.Min(Math.Pow((Mass1Position - Mass2Position).Length, 2), 10000);
            ...
            foreach (var mass in Masses)
            {
                ...
                i++;
                mass.ApplyForce(new Vector((count/2 - i) / count * lengthSquared, 0)); //The air friction
            }
        }
